...
| `seed.go` | Loads `brand.json` and `fixtures/` into Postgres. |
| `replay.go` | Shifts fixture timestamps so the corpus ends "now". |
| `inject_crisis.go` | Drops 40 synthetic negative mentions to fire the crisis rule. |
| `record_fixtures.go` | The one live Anakin recording session. B2 only. |
| `run_demo.sh` | The full flow. `--reset` reseeds, `--live` adds the live call. |

Each Go entry above is its own `package main` in its own subdirectory
(`demo/seed/`, `demo/replay/`, and so on) and runs as `go run ./demo/seed`.
Four `func main` in one package does not compile, so the table names the
command, not the file layout.

## Invariants and gotchas

- **`record_fixtures.go` is the only irreversible thing in this repo.** It
  prints a dry-run credit estimate and exits unless `--confirm` is passed. 300
  free credits, spent once.
- **`replay.go` is deterministic.** Same fixtures in, same timestamps out, every
